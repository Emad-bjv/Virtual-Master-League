...
for th in TransferHistory.objects.filter(id__gte=585, id__lte=605).order_by('id'):
    p_name = th.player.name if th.player else "None"
    s_name = th.seller_team.name if th.seller_team else "None"
    b_name = th.buyer_team.name if th.buyer_team else "None"
    print(f"{th.id}: [{th.transferred_at}] {p_name} ({th.player_id}) | {s_name} -> {b_name} | {th.transfer_type}")
"""
