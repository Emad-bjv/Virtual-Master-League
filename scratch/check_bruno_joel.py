...
for name in ['Bruno Guimarães', 'Joelinton']:
    p = Player.objects.filter(name__icontains=name).first()
    print(f"\\n=== {p.name} (ID {p.id}) Current: {p.team.name if p.team else None} ===")
    for l in TransferLog.objects.filter(description__icontains=p.name)[:8]:
